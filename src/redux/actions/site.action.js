import { SITE, HEADERS } from "./_constants";
...
export function setLoadingWa(load) {
  return {
    type: SITE.LOADING_WA,
    load,
  };
}
export function setSiteWa(data = []) {
  return {
    type: SITE.DATA_WA,
    data,
  };
...
export const FetchLogWa = (where) => {
  return (dispatch) => {
    dispatch(setLoadingWa(true));
    let url = "site/log_wa";
    if (where !== "") url += `?${where}`;
    axios
      .get(HEADERS.URL + url)
      .then(function (response) {
        ...
        dispatch(setSiteWa(data));
        dispatch(setLoadingWa(false));
      })
