...
class PinRo extends Component {
  ...
  componentWillMount() {
    this.props.dispatch(getKategoriPin(`page=1&type=ro`));
    this.setState({ dataPin: [] });
  }

  handleOnchange(sct) {
    this.props.dispatch(
      getPinRo(`category=${this.props.dataKategori[sct].id}`)
    );
    this.setState({
      section: sct,
...
const mapStateToProps = (state) => {
  return {
    ...
    isLoading: state.pinReducer.isLoadingRo,
    data: state.pinReducer.dataRo,
