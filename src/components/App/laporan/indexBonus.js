...
import {
  ...
  postPenarikan,
  getExcelBonus,
} from "../../../redux/actions/laporan/bonus.action";
...
class IndexBonus extends Component {
  ...
  getExcel(props) {
    if (props.dataExcel !== undefined) {
      if (props.dataExcel.length > 0) {
        let content = [];
        let ttl = this.state.tipe === "aktivasi" ? "AKTIVASI" : "RO";
  ...
  printDocumentXLsx(e) {
    e.preventDefault();
    const tipe = this.state.tipe === "aktivasi" ? "&type=aktivasi" : "&type=ro";
    ...
    return (
      <Layout page={"Transaksi Bonus"}>
        <div className="row align-items-center">
          ...
          <div className="col-6">
            <div
              className="dashboard-header-title mb-3"
            ...
            >
              <button
                ...
                type="button"
                className={
                  this.state.tipe === "aktivasi"
                    ? "btn btn-success mb-2 mr-2"
                    : "btn btn-outline-success mb-2 mr-2"
                }
                onClick={(e) => this.handleTipe(e, "aktivasi")}
              >
                Bonus Aktivasi/Registrasi
              </button>
              <button
                style={{ marginTop: "27px" }}
                type="button"
                ...
                onClick={(e) => this.handleTipe(e, "ro")}
              >
                Bonus Royalti/Repeat Order
              </button>
              <button
                style={{ marginTop: "27px", marginLeft: "5px" }}
                className="btn btn-primary mb-2 mr-2"
                onClick={(e) => this.printDocumentXLsx(e)}
              >
                ...
                <table className="table table-bordered">
                  <thead className="thead-light">
                    <tr>
                      ...
                      <th colSpan={3} style={columnStyle}>
                        Transaksi
                      </th>
                      <th rowSpan={2} style={columnStyle}>
                        Aksi
                  ...
                  <tbody>
                    {this.state.isLoading === false ? (
                      this.state.data.length > 0 ? (
                        this.state.data.map((v, i) => {
                          return (
                            <tr key={i}>
                              ...
                              <td>{v.percentage} %</td>
                              <td>
                                {parseInt(v.trx_in, 10) === 0
                                  ? 0
                                  : toCurrency(v.trx_in)}
                              </td>
                              <td>
                                {parseInt(v.trx_out, 10) === 0
                                  ? 0
                                  : toCurrency(v.trx_out)}
                              </td>
                              <td>
                                {parseInt(v.total, 10) === 0
                                  ? 0
                                  : toCurrency(v.total)}
                              </td>
                              <td>
                                <button
                                  onClick={(e) => this.handleModal(e, v.kode)}
                        ...
                        <tr>
                          <td colSpan={7} style={columnStyle}>
