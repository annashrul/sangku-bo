...
class FormPaket extends Component {
  ...
  handleRemove(e, i, u) {
    if (u === this.state.detail[i].uniq) {
      let someArray = this.removeItemOnce(
        this.state.detail,
        this.state.detail[i]
      );
      let hrg = parseInt(this.state.harga, 10);
    ...
    return (
      ...
      >
        <ModalHeader toggle={this.toggle}>
          {this.props.detail.id === "" ? "Tambah Paket" : "Ubah Paket"}
        </ModalHeader>
        <ModalBody>
          <div className="row">
              ...
              >
                <div id="chat_user_2">
                  <ul className="chat-list list-unstyled">
                    {this.state.barang_data.length > 0 ? (
                      this.state.barang_data.map((v, i) => {
                        ...
                        return (
                          <li
                            // style={bg}
                            onClick={(event) => this.handleAddBarang(event, i)}
                            id={`item${i}`}
                            className="clearfix"
                            key={i}
                          >
                            <span className="circle" style={{ float: "left" }}>
                              {i + 1}
                            </span>
                            <span className="circle" style={{ float: "right" }}>
                          ...
                          <tr key={i}>
                            ...
                            <td style={columnStyle}>
                              <button
                                ...
                                onClick={(event) =>
                                  this.handleRemove(event, i, v.uniq)
                                }
                              >
